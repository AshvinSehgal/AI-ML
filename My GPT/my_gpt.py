...
ds = load_dataset("HuggingFaceTB/cosmopedia-100k", "web_samples_v1")['train']['text']
batch_size = 5
def batch_iterator():
...
# tokenizer.train_new_from_iterator(ds, vocab_size = hparams.n_vocab)
# tokenizer.save_pretrained("my_tokenizer")

# tokenizer = Tokenizer(models.BPE())
# tokenizer.pre_tokenizer = pre_tokenizers.ByteLevel(add_prefix_space=False)
# tokenizer.pre_tokenizer.pre_tokenize_str("This is an example!")
# trainer = trainers.BpeTrainer(vocab_size=hparams.n_vocab, special_tokens=["<|endoftext|>"])
...
input_ids = []
for doc in ds:
    input_ids.append(torch.tensor(tokenizer.encode(doc)))
input_ids = pad_sequence(input_ids, batch_first=True, padding_value=0)

outputs = model(input_ids, past=None)
